...
package Board_Type_Decode with SPARK_Mode is
   ...
   function Extract_Upi (Serial : Serial64) return Serial64
     is (Shift_Right (Serial, 36) and 16#FFFFF#)
     with Post => Extract_Upi'Result = (Shift_Right (Serial, 36) and 16#FFFFF#)
                  and then Extract_Upi'Result <= 16#FFFFF#;

   function Decode_Board_Type (Serial : Serial64) return Board_Type
     is (case Extract_Upi (Serial) is
     ...
     with Post => (if (Shift_Right (Serial, 36) and 16#FFFFF#) = 16#43# then Decode_Board_Type'Result = Board_P100a)
                  and then (if (Shift_Right (Serial, 36) and 16#FFFFF#) = 16#202# then Decode_Board_Type'Result = Board_Galaxy_Bh)
                  and then (if (Shift_Right (Serial, 36) and 16#FFFFF#) = 16#3# then Decode_Board_Type'Result = Board_E150);
